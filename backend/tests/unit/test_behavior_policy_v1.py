...
def test_can_initiate_initiated_today_block(db_session, behavior_user_id):
    ...
    with patch("backend.app.behavior.policy.is_within_quiet_hours", return_value=False):
        ...
        allowed, reason = policy.can_initiate(
            db_session, behavior_user_id, datetime.utcnow(),
...
def test_can_initiate_daily_cap_block(db_session, behavior_user_id):
...
def test_can_initiate_cooldown_block(db_session, behavior_user_id):
...
def test_can_initiate_quiet_hours_block(db_session, behavior_user_id):
...
def test_can_initiate_allowed(db_session, behavior_user_id):
...
@pytest.fixture
def db_session():
    """In-memory SQLite session for policy tests (quiet_hours is patched in tests that need it)."""
