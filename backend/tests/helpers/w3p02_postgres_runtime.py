"""W3-P02 GitHub PostgreSQL weekly orchestrator runtime helper.
...
EXPECTED_PORT = 5432
EXPECTED_DATABASE = "sedi_w3p02_orchestrator"
EXPECTED_USER = "sedi_w3p02_test"
EXPECTED_PASSWORD = "sedi_w3p02_test_password"
EXPECTED_TEST_URL = (
...
W3P02_EXPECTED_SCHEMA_TABLES: tuple[str, ...] = (
...
_W3P02_TEST = "backend/tests/test_section30_i5_w3_p02_weekly_orchestrator.py"

W3P02_RUNTIME_SELECTORS: tuple[str, ...] = (
    f"{_W3P02_TEST}::test_W3P02_T1_package_identity",
    f"{_W3P02_TEST}::test_W3P02_T2_activation_off_and_dormant_scheduler_contract",
    f"{_W3P02_TEST}::test_W3P02_T3_discovery_select_eligible",
    f"{_W3P02_TEST}::test_W3P02_T4_governance_fail_closed",
    f"{_W3P02_TEST}::test_W3P02_T5_adapter_resolve_and_unknown",
    f"{_W3P02_TEST}::test_W3P02_T6_discovery_dedupe",
    f"{_W3P02_TEST}::test_W3P02_T7_create_run_idempotency",
    f"{_W3P02_TEST}::test_W3P02_T8_attempt_transitions_and_invalid",
    f"{_W3P02_TEST}::test_W3P02_T9_retry_after_failure",
    f"{_W3P02_TEST}::test_W3P02_T10_source_result_lifecycle",
    f"{_W3P02_TEST}::test_W3P02_T11_gap_result_lifecycle",
    f"{_W3P02_TEST}::test_W3P02_T12_partial_full_no_eligible",
    f"{_W3P02_TEST}::test_W3P02_T13_handoff_prepare_only",
    f"{_W3P02_TEST}::test_W3P02_T14_dry_run_no_activation_no_production_write",
    f"{_W3P02_TEST}::test_W3P02_T15_source_fk_invalid",
)

W3P02_EXPECTED_RUNTIME_NODE_IDS: frozenset[str] = frozenset(
    (
        f"{_W3P02_TEST}::test_W3P02_T1_package_identity",
        f"{_W3P02_TEST}::test_W3P02_T2_activation_off_and_dormant_scheduler_contract",
        f"{_W3P02_TEST}::test_W3P02_T3_discovery_select_eligible",
        f"{_W3P02_TEST}::test_W3P02_T4_governance_fail_closed[unknown_rights]",
        f"{_W3P02_TEST}::test_W3P02_T4_governance_fail_closed[robots_blocked]",
        f"{_W3P02_TEST}::test_W3P02_T4_governance_fail_closed[rate_undefined]",
        f"{_W3P02_TEST}::test_W3P02_T4_governance_fail_closed[registry_blocked]",
        f"{_W3P02_TEST}::test_W3P02_T5_adapter_resolve_and_unknown",
        f"{_W3P02_TEST}::test_W3P02_T6_discovery_dedupe",
        f"{_W3P02_TEST}::test_W3P02_T7_create_run_idempotency",
        f"{_W3P02_TEST}::test_W3P02_T8_attempt_transitions_and_invalid",
        ...
        f"{_W3P02_TEST}::test_W3P02_T10_source_result_lifecycle",
        f"{_W3P02_TEST}::test_W3P02_T11_gap_result_lifecycle",
        f"{_W3P02_TEST}::test_W3P02_T12_partial_full_no_eligible[partial]",
        f"{_W3P02_TEST}::test_W3P02_T12_partial_full_no_eligible[full_failure]",
        f"{_W3P02_TEST}::test_W3P02_T12_partial_full_no_eligible[no_eligible]",
        f"{_W3P02_TEST}::test_W3P02_T13_handoff_prepare_only",
        f"{_W3P02_TEST}::test_W3P02_T14_dry_run_no_activation_no_production_write",
        f"{_W3P02_TEST}::test_W3P02_T15_source_fk_invalid",
    )
...
@dataclass
class W3P02GitHubPostgresEvidence:
    mode: str = ""
    run_id: str = ""
    workflow_sha: str = ""
    commit_sha: str = ""
...
class W3P02HarnessError(RuntimeError):
...
def assert_no_db_bootstrap_in_collect_only(collect_only_mode: bool) -> None:
    if collect_only_mode:
        raise W3P02HarnessError(SENTINEL_RUNTIME_BOOTSTRAP_FORBIDDEN_IN_COLLECT_ONLY)


def scrub_inherited_database_env() -> None:
    for key in (
        "SEDI_W3P02_PG_ADMIN_URL",
        "SEDI_W3P02_PG_TEST_URL",
        "SEDI_W3P02_PG_OWNERSHIP_MARKER",
    ):
...
def refuse_production_shared_targets(test_url: str) -> None:
    ...
    for token in PRODUCTION_REFUSE_SUBSTRINGS:
        if token in lower:
            raise W3P02HarnessError(SENTINEL_GITHUB_POSTGRES_PRODUCTION_IDENTIFIER_REFUSED)
    parsed = urlparse(test_url)
    host = (parsed.hostname or "").lower()
    if host in {"localhost", "::1"}:
        raise W3P02HarnessError(SENTINEL_GITHUB_POSTGRES_TARGET_REFUSED)
...
def bind_github_database_environment(test_url: str) -> None:
    ...
    os.environ.setdefault("OPENAI_API_KEY", "w3p02-test-placeholder-not-used")
...
def verify_bound_environment(expected_db_name: str, expected_host: str) -> None:
    ...
    if not db_url or not test_url:
        raise W3P02HarnessError(SENTINEL_GITHUB_POSTGRES_URL_MISMATCH)
    if db_url != test_url:
        raise W3P02HarnessError(SENTINEL_GITHUB_POSTGRES_URL_MISMATCH)
...
def assert_import_guard_completed() -> None:
    if not _IMPORT_GUARD_COMPLETE:
        raise W3P02HarnessError(SENTINEL_APPLICATION_IMPORT_BEFORE_TEST_DATABASE_BINDING)
...
def verify_runtime_selector_manifest(
    ...
    expected = expected_ids or W3P02_EXPECTED_RUNTIME_NODE_IDS
    ...
    if len(collected) != EXPECTED_RUNTIME_NODE_COUNT:
        raise W3P02HarnessError(SENTINEL_RUNTIME_NODE_MANIFEST_MISMATCH)
...
def verify_w3p02_schema_tables(engine, expected_tables: tuple[str, ...]) -> None:
...
def set_active_node_id(state: W3P02PluginState, node_id: str) -> None:
    state.active_node_id = node_id


def clear_active_node_id(state: W3P02PluginState) -> None:
    state.active_node_id = None


def correlate_diagnostic_to_node(
    state: W3P02PluginState, node_id: str, diag: dict[str, Any]
) -> dict[str, Any]:
...
def record_correlated_diagnostic(state: W3P02PluginState, diag: dict[str, Any]) -> None:
...
def _make_handle_error_listener(state: W3P02PluginState):
...
def register_handle_error_listener(engine, state: W3P02PluginState) -> None:
...
def remove_handle_error_listener(engine, state: W3P02PluginState) -> None:
...
def verify_sqlalchemy_version_minimum() -> None:
    ...
    try:
        ...
        major = 0
    if major < 2:
        raise W3P02HarnessError(BLOCKED_DEPENDENCY_RESOLUTION)
...
def resolve_runner_temp_dir(repo_root: Optional[Path] = None) -> Path:
    ...
    target = (Path(tempfile.gettempdir()) / "sedi_w3p02_postgres_runtime_evidence").resolve()
    target.mkdir(parents=True, exist_ok=True)
    if _path_is_inside(target, repo_root):
        raise W3P02HarnessError("FAIL_UNAUTHORIZED_REPOSITORY_MUTATION evidence path in repo")
    return target
...
def capture_redacted_evidence(evidence: W3P02GitHubPostgresEvidence) -> dict[str, Any]:
    return {
        "mode": evidence.mode,
...
def initialize_evidence_context(state: W3P02PluginState, *, mode: str) -> None:
...
def configure_environment_from_urls(state: W3P02PluginState) -> None:
...
def runtime_bootstrap_after_manifest(state: W3P02PluginState) -> None:
    assert_no_db_bootstrap_in_collect_only(state.collect_only_mode)
    ...
    verify_w3p02_schema_tables(engine, W3P02_EXPECTED_SCHEMA_TABLES)
    register_handle_error_listener(engine, state)
    state.engine = engine
    state.evidence.schema_tables = sorted(
        t for t in tables if t in set(W3P02_EXPECTED_SCHEMA_TABLES) or True
    )
    state.runtime_bootstrapped = True


def independent_db_resource_cleanup(
    state: W3P02PluginState,
    *,
    session: Any = None,
