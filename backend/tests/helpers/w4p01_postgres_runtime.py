"""W4-P01 GitHub PostgreSQL knowledge retrieval runtime helper.
...
EXPECTED_PORT = 5432
EXPECTED_DATABASE = "sedi_w4p01_orchestrator"
EXPECTED_USER = "sedi_w4p01_test"
EXPECTED_PASSWORD = "sedi_w4p01_test_password"
EXPECTED_TEST_URL = (
...
W4P01_EXPECTED_SCHEMA_TABLES: tuple[str, ...] = (
    "knowledge_units",
...
W4P01_RUNTIME_SELECTORS: tuple[str, ...] = (
    ...
    f"{_W4P01_TEST}::test_W4P01_T2_query_normalization",
    f"{_W4P01_TEST}::test_W4P01_T3_eligible_current_returned",
    f"{_W4P01_TEST}::test_W4P01_T4_superseded_excluded",
    f"{_W4P01_TEST}::test_W4P01_T5_retracted_excluded",
    f"{_W4P01_TEST}::test_W4P01_T6_missing_provenance_excluded",
    f"{_W4P01_TEST}::test_W4P01_T7_stale_excluded",
    f"{_W4P01_TEST}::test_W4P01_T8_conflict_excluded",
    f"{_W4P01_TEST}::test_W4P01_T9_medical_safety_excluded",
    f"{_W4P01_TEST}::test_W4P01_T10_ranking_deterministic",
    f"{_W4P01_TEST}::test_W4P01_T11_no_safe_knowledge_gap_enqueue",
    f"{_W4P01_TEST}::test_W4P01_T12_language_filter_personalization",
    f"{_W4P01_TEST}::test_W4P01_T13_no_base_model_fallback_marker",
...
W4P01_EXPECTED_RUNTIME_NODE_IDS: frozenset[str] = frozenset(
    (
        ...
        f"{_W4P01_TEST}::test_W4P01_T6_missing_provenance_excluded",
        f"{_W4P01_TEST}::test_W4P01_T7_stale_excluded",
        f"{_W4P01_TEST}::test_W4P01_T8_conflict_excluded[suspected]",
        f"{_W4P01_TEST}::test_W4P01_T8_conflict_excluded[confirmed]",
        f"{_W4P01_TEST}::test_W4P01_T9_medical_safety_excluded[pending_review]",
        f"{_W4P01_TEST}::test_W4P01_T9_medical_safety_excluded[restricted]",
        f"{_W4P01_TEST}::test_W4P01_T9_medical_safety_excluded[blocked]",
        f"{_W4P01_TEST}::test_W4P01_T10_ranking_deterministic",
        f"{_W4P01_TEST}::test_W4P01_T11_no_safe_knowledge_gap_enqueue",
        f"{_W4P01_TEST}::test_W4P01_T12_language_filter_personalization",
        f"{_W4P01_TEST}::test_W4P01_T13_no_base_model_fallback_marker",
...
@dataclass
class W4P01GitHubPostgresEvidence:
    mode: str = ""
    run_id: str = ""
    workflow_sha: str = ""
    commit_sha: str = ""
...
class W4P01HarnessError(RuntimeError):
...
def assert_no_db_bootstrap_in_collect_only(collect_only_mode: bool) -> None:
    if collect_only_mode:
        raise W4P01HarnessError(SENTINEL_RUNTIME_BOOTSTRAP_FORBIDDEN_IN_COLLECT_ONLY)


def scrub_inherited_database_env() -> None:
    for key in (
        "SEDI_W4P01_PG_ADMIN_URL",
        "SEDI_W4P01_PG_TEST_URL",
        "SEDI_W4P01_PG_OWNERSHIP_MARKER",
    ):
...
def refuse_production_shared_targets(test_url: str) -> None:
    ...
    for token in PRODUCTION_REFUSE_SUBSTRINGS:
        if token in lower:
            raise W4P01HarnessError(SENTINEL_GITHUB_POSTGRES_PRODUCTION_IDENTIFIER_REFUSED)
    parsed = urlparse(test_url)
    host = (parsed.hostname or "").lower()
    if host in {"localhost", "::1"}:
        raise W4P01HarnessError(SENTINEL_GITHUB_POSTGRES_TARGET_REFUSED)
...
def bind_github_database_environment(test_url: str) -> None:
    ...
    os.environ.setdefault("OPENAI_API_KEY", "w4p01-test-placeholder-not-used")
...
def verify_bound_environment(expected_db_name: str, expected_host: str) -> None:
    ...
    if not db_url or not test_url:
        raise W4P01HarnessError(SENTINEL_GITHUB_POSTGRES_URL_MISMATCH)
    if db_url != test_url:
        raise W4P01HarnessError(SENTINEL_GITHUB_POSTGRES_URL_MISMATCH)
...
def assert_import_guard_completed() -> None:
    if not _IMPORT_GUARD_COMPLETE:
        raise W4P01HarnessError(SENTINEL_APPLICATION_IMPORT_BEFORE_TEST_DATABASE_BINDING)
...
def verify_runtime_selector_manifest(
    ...
    expected = expected_ids or W4P01_EXPECTED_RUNTIME_NODE_IDS
    ...
    if len(collected) != EXPECTED_RUNTIME_NODE_COUNT:
        raise W4P01HarnessError(SENTINEL_RUNTIME_NODE_MANIFEST_MISMATCH)
...
def verify_w4p01_schema_tables(engine, expected_tables: tuple[str, ...]) -> None:
...
def set_active_node_id(state: W4P01PluginState, node_id: str) -> None:
    state.active_node_id = node_id


def clear_active_node_id(state: W4P01PluginState) -> None:
    state.active_node_id = None


def correlate_diagnostic_to_node(
    state: W4P01PluginState, node_id: str, diag: dict[str, Any]
) -> dict[str, Any]:
...
def record_correlated_diagnostic(state: W4P01PluginState, diag: dict[str, Any]) -> None:
...
def _make_handle_error_listener(state: W4P01PluginState):
...
def register_handle_error_listener(engine, state: W4P01PluginState) -> None:
...
def remove_handle_error_listener(engine, state: W4P01PluginState) -> None:
...
def verify_sqlalchemy_version_minimum() -> None:
    ...
    try:
        ...
        major = 0
    if major < 2:
        raise W4P01HarnessError(BLOCKED_DEPENDENCY_RESOLUTION)
...
def resolve_runner_temp_dir(repo_root: Optional[Path] = None) -> Path:
    ...
    target = (Path(tempfile.gettempdir()) / "sedi_w4p01_postgres_runtime_evidence").resolve()
    target.mkdir(parents=True, exist_ok=True)
    if _path_is_inside(target, repo_root):
        raise W4P01HarnessError("FAIL_UNAUTHORIZED_REPOSITORY_MUTATION evidence path in repo")
    return target
...
def capture_redacted_evidence(evidence: W4P01GitHubPostgresEvidence) -> dict[str, Any]:
    return {
        "mode": evidence.mode,
...
def initialize_evidence_context(state: W4P01PluginState, *, mode: str) -> None:
...
def configure_environment_from_urls(state: W4P01PluginState) -> None:
...
def runtime_bootstrap_after_manifest(state: W4P01PluginState) -> None:
    assert_no_db_bootstrap_in_collect_only(state.collect_only_mode)
    ...
    verify_w4p01_schema_tables(engine, W4P01_EXPECTED_SCHEMA_TABLES)
    register_handle_error_listener(engine, state)
    state.engine = engine
    state.evidence.schema_tables = sorted(
        t for t in tables if t in set(W4P01_EXPECTED_SCHEMA_TABLES) or True
    )
    state.runtime_bootstrapped = True


def independent_db_resource_cleanup(
    state: W4P01PluginState,
    *,
    session: Any = None,
