"""W5-P01 GitHub PostgreSQL iran directory runtime helper.
...
EXPECTED_PORT = 5432
EXPECTED_DATABASE = "sedi_w5p01_directory"
EXPECTED_USER = "sedi_w5p01_test"
EXPECTED_PASSWORD = "sedi_w5p01_test_password"
EXPECTED_TEST_URL = (
...
W5P01_EXPECTED_SCHEMA_TABLES: tuple[str, ...] = (
    "iran_doctors",
    "iran_laboratories",
    "iran_hospitals",
    "user_doctors",
...
_W5P01_TEST = "backend/tests/test_section30_i5_w5_iran_directory.py"

W5P01_RUNTIME_SELECTORS: tuple[str, ...] = (
    "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T1_package_identity",
    "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T2_iran_doctor_orm_persist",
    "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T3_iran_laboratory_orm_persist",
    "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T4_iran_hospital_orm_persist",
    "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T5_unique_canonical_directory_key",
    "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T6_search_doctor",
    "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T7_search_laboratory",
    "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T8_search_hospital",
    "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T9_search_filter_city_specialty",
    "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T10_empty_result",
    "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T11_schema_serialization",
    "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T12_api_contract",
    "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T13_entity_type_separation",
    "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T14_userdoctor_vs_irandoctor_separation",
    "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T15_no_ir_to_ku_structural_fk",
    "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T16_no_ku_creation_via_directory_service",
    "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T17_no_clinical_evidence_output",
    "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T18_no_network_markers",
    "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T19_migration_authored_not_run",
    "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T20_source_legal_boundary_no_live_source",
    "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T21_endorsement_disclaimer",
    "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T22_git_history_evidence_invariant",
    "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T23_medical_center_facility_type",
    "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T24_inactive_excluded_by_default",
)

W5P01_EXPECTED_RUNTIME_NODE_IDS: frozenset[str] = frozenset(
    (
        "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T1_package_identity",
        "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T2_iran_doctor_orm_persist",
        "backend/tests/test_section30_i5_w5_iran_directory.py::test_W5P01_T3_iran_laboratory_orm_persist",
...
EXPECTED_RUNTIME_NODE_COUNT = 24
EXPECTED_SELECTOR_COUNT = 24
...
@dataclass
class W5P01GitHubPostgresEvidence:
    mode: str = ""
    run_id: str = ""
    workflow_sha: str = ""
    commit_sha: str = ""
...
class W5P01HarnessError(RuntimeError):
...
def assert_no_db_bootstrap_in_collect_only(collect_only_mode: bool) -> None:
    if collect_only_mode:
        raise W5P01HarnessError(SENTINEL_RUNTIME_BOOTSTRAP_FORBIDDEN_IN_COLLECT_ONLY)


def scrub_inherited_database_env() -> None:
    for key in (
        "SEDI_W5P01_PG_ADMIN_URL",
        "SEDI_W5P01_PG_TEST_URL",
        "SEDI_W5P01_PG_OWNERSHIP_MARKER",
    ):
...
def refuse_production_shared_targets(test_url: str) -> None:
    ...
    for token in PRODUCTION_REFUSE_SUBSTRINGS:
        if token in lower:
            raise W5P01HarnessError(SENTINEL_GITHUB_POSTGRES_PRODUCTION_IDENTIFIER_REFUSED)
    parsed = urlparse(test_url)
    host = (parsed.hostname or "").lower()
    if host in {"localhost", "::1"}:
        raise W5P01HarnessError(SENTINEL_GITHUB_POSTGRES_TARGET_REFUSED)
...
def bind_github_database_environment(test_url: str) -> None:
    ...
    os.environ.setdefault("OPENAI_API_KEY", "w5p01-test-placeholder-not-used")
...
def verify_bound_environment(expected_db_name: str, expected_host: str) -> None:
    ...
    if not db_url or not test_url:
        raise W5P01HarnessError(SENTINEL_GITHUB_POSTGRES_URL_MISMATCH)
    if db_url != test_url:
        raise W5P01HarnessError(SENTINEL_GITHUB_POSTGRES_URL_MISMATCH)
...
def assert_import_guard_completed() -> None:
    if not _IMPORT_GUARD_COMPLETE:
        raise W5P01HarnessError(SENTINEL_APPLICATION_IMPORT_BEFORE_TEST_DATABASE_BINDING)
...
def verify_runtime_selector_manifest(
    ...
    expected = expected_ids or W5P01_EXPECTED_RUNTIME_NODE_IDS
    ...
    if len(collected) != EXPECTED_RUNTIME_NODE_COUNT:
        raise W5P01HarnessError(SENTINEL_RUNTIME_NODE_MANIFEST_MISMATCH)
...
def verify_w5p01_schema_tables(engine, expected_tables: tuple[str, ...]) -> None:
...
def set_active_node_id(state: W5P01PluginState, node_id: str) -> None:
    state.active_node_id = node_id


def clear_active_node_id(state: W5P01PluginState) -> None:
    state.active_node_id = None


def correlate_diagnostic_to_node(
    state: W5P01PluginState, node_id: str, diag: dict[str, Any]
) -> dict[str, Any]:
...
def record_correlated_diagnostic(state: W5P01PluginState, diag: dict[str, Any]) -> None:
...
def _make_handle_error_listener(state: W5P01PluginState):
...
def register_handle_error_listener(engine, state: W5P01PluginState) -> None:
...
def remove_handle_error_listener(engine, state: W5P01PluginState) -> None:
...
def verify_sqlalchemy_version_minimum() -> None:
    ...
    try:
        ...
        major = 0
    if major < 2:
        raise W5P01HarnessError(BLOCKED_DEPENDENCY_RESOLUTION)
...
def resolve_runner_temp_dir(repo_root: Optional[Path] = None) -> Path:
    ...
    target = (Path(tempfile.gettempdir()) / "sedi_w5p01_postgres_runtime_evidence").resolve()
    target.mkdir(parents=True, exist_ok=True)
    if _path_is_inside(target, repo_root):
        raise W5P01HarnessError("FAIL_UNAUTHORIZED_REPOSITORY_MUTATION evidence path in repo")
    return target
...
def capture_redacted_evidence(evidence: W5P01GitHubPostgresEvidence) -> dict[str, Any]:
    return {
        "mode": evidence.mode,
...
def initialize_evidence_context(state: W5P01PluginState, *, mode: str) -> None:
...
def configure_environment_from_urls(state: W5P01PluginState) -> None:
...
def runtime_bootstrap_after_manifest(state: W5P01PluginState) -> None:
    assert_no_db_bootstrap_in_collect_only(state.collect_only_mode)
