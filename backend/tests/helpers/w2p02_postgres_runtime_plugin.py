"""W2-P01 GitHub PostgreSQL knowledge safety runtime pytest plugin."""
...
import backend.tests.helpers.w2p02_postgres_runtime as rt

STATE_ATTR = "_w2p02_plugin_state"


def _get_state(config: pytest.Config) -> rt.W2P02PluginState:
    state = getattr(config, STATE_ATTR, None)
    if state is None:
        state = rt.W2P02PluginState()
...
def pytest_collection_modifyitems(
    ...
    except rt.W2P02HarnessError as exc:
...
def w2p02_github_postgres_evidence(pytestconfig: pytest.Config) -> rt.W2P02GitHubPostgresEvidence:
    if pytestconfig.getoption("collectonly"):
        pytest.skip("w2p02_github_postgres_evidence unavailable in collect-only mode")
...
@pytest.fixture(scope="session")
def w2p02_postgres_engine(pytestconfig: pytest.Config):
    if pytestconfig.getoption("collectonly"):
        pytest.skip("w2p02_postgres_engine unavailable in collect-only mode")
    state = _get_state(pytestconfig)
    rt.refuse_schema_start_after_manifest_mismatch(state.manifest_pass)
...
@pytest.fixture()
def db(request: pytest.FixtureRequest, w2p02_postgres_engine) -> Generator[Any, None, None]:
    ...
    session = None
    try:
        connection = w2p02_postgres_engine.connect()
        ...
        if cleanup_errors and not _prior_primary_failure_exists(request):
            ...
            raise rt.W2P02HarnessError(rt.CONTROLLED_FIXTURE_CLEANUP_FAILED)
...
def pytest_sessionfinish(session: pytest.Session, exitstatus: int) -> None:
    ...
    try:
        ...
        out = evidence_dir / (
            "w2p02_collect_only_evidence.json"
            if state.collect_only_mode
            else "w2p02_runtime_evidence.json"
        )
