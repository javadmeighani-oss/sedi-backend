"""CAP17 GitHub PostgreSQL personalization runtime pytest plugin.

Reuses W4-P01 postgres bootstrap/fixtures while enforcing CAP17 node manifest.
"""
...
import backend.tests.helpers.w4p01_cap17_personalization_runtime as cap17
import backend.tests.helpers.w4p01_postgres_runtime as rt

# Enforce CAP17 manifest for this plugin process.
rt.W4P01_EXPECTED_RUNTIME_NODE_IDS = cap17.CAP17_EXPECTED_RUNTIME_NODE_IDS
rt.EXPECTED_RUNTIME_NODE_COUNT = cap17.EXPECTED_CAP17_NODE_COUNT
rt.EXPECTED_SELECTOR_COUNT = cap17.EXPECTED_CAP17_SELECTOR_COUNT
rt.W4P01_RUNTIME_SELECTORS = cap17.CAP17_RUNTIME_SELECTORS

STATE_ATTR = "_w4p01_cap17_plugin_state"


def _get_state(config: pytest.Config) -> rt.W4P01PluginState:
    state = getattr(config, STATE_ATTR, None)
    if state is None:
        state = rt.W4P01PluginState()
...
def pytest_collection_modifyitems(
    ...
    try:
        verified = rt.verify_runtime_selector_manifest(
            collected_ids, expected_ids=cap17.CAP17_EXPECTED_RUNTIME_NODE_IDS
        )
        # verify_runtime_selector_manifest still checks EXPECTED_RUNTIME_NODE_COUNT
        state.manifest_pass = True
        state.evidence.manifest_equality_pass = True
        state.evidence.collected_node_ids = verified
    except rt.W4P01HarnessError:
...
def w4p01_github_postgres_evidence(pytestconfig: pytest.Config) -> rt.W4P01GitHubPostgresEvidence:
    if pytestconfig.getoption("collectonly"):
        raise rt.W4P01HarnessError("EVIDENCE_FIXTURE_FORBIDDEN_IN_COLLECT_ONLY")
...
@pytest.fixture(scope="session")
def w4p01_postgres_engine(pytestconfig: pytest.Config):
    if pytestconfig.getoption("collectonly"):
        raise rt.W4P01HarnessError("ENGINE_FIXTURE_FORBIDDEN_IN_COLLECT_ONLY")
    state = _get_state(pytestconfig)
    rt.refuse_schema_start_after_manifest_mismatch(state.manifest_pass)
...
@pytest.fixture()
def db(request: pytest.FixtureRequest, w4p01_postgres_engine) -> Generator[Any, None, None]:
    state = _get_state(request.config)
    connection = None
    outer_transaction = None
    session = None
    try:
        connection = w4p01_postgres_engine.connect()
        ...
        if cleanup_errors and not _prior_primary_failure_exists(request):
            raise rt.W4P01HarnessError(rt.CONTROLLED_FIXTURE_CLEANUP_FAILED)
...
def pytest_sessionfinish(session: pytest.Session, exitstatus: int) -> None:
    ...
    payload["cap17_manifest"] = True
    try:
        evidence_dir = rt.resolve_runner_temp_dir()
        out = evidence_dir / (
            "w4p01_cap17_collect_only_evidence.json"
            if state.collect_only_mode
            else "w4p01_cap17_runtime_evidence.json"
        )
        rt.write_runner_temp_evidence(out, payload)
    except rt.W4P01HarnessError as exc:
