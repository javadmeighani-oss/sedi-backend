"""W3-P02 GitHub PostgreSQL weekly orchestrator runtime pytest plugin."""
...
import backend.tests.helpers.w3p02_postgres_runtime as rt

STATE_ATTR = "_w3p02_plugin_state"


def _get_state(config: pytest.Config) -> rt.W3P02PluginState:
    state = getattr(config, STATE_ATTR, None)
    if state is None:
        state = rt.W3P02PluginState()
...
def pytest_collection_modifyitems(
    ...
    except rt.W3P02HarnessError as exc:
...
def w3p02_github_postgres_evidence(pytestconfig: pytest.Config) -> rt.W3P02GitHubPostgresEvidence:
    if pytestconfig.getoption("collectonly"):
        raise rt.W3P02HarnessError("EVIDENCE_FIXTURE_FORBIDDEN_IN_COLLECT_ONLY")
...
@pytest.fixture(scope="session")
def w3p02_postgres_engine(pytestconfig: pytest.Config):
    if pytestconfig.getoption("collectonly"):
        raise rt.W3P02HarnessError("ENGINE_FIXTURE_FORBIDDEN_IN_COLLECT_ONLY")
    state = _get_state(pytestconfig)
    rt.refuse_schema_start_after_manifest_mismatch(state.manifest_pass)
...
@pytest.fixture()
def db(request: pytest.FixtureRequest, w3p02_postgres_engine) -> Generator[Any, None, None]:
    state = _get_state(request.config)
    connection = None
    outer_transaction = None
    session = None
    try:
        connection = w3p02_postgres_engine.connect()
        ...
        if cleanup_errors and not _prior_primary_failure_exists(request):
            raise rt.W3P02HarnessError(rt.CONTROLLED_FIXTURE_CLEANUP_FAILED)
...
def pytest_sessionfinish(session: pytest.Session, exitstatus: int) -> None:
    ...
    try:
        ...
        out = evidence_dir / (
            "w3p02_collect_only_evidence.json"
            if state.collect_only_mode
            else "w3p02_runtime_evidence.json"
        )
