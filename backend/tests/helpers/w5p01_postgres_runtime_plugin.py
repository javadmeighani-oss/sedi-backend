"""W5-P01 GitHub PostgreSQL grounded synthesis runtime pytest plugin."""
...
import backend.tests.helpers.w5p01_postgres_runtime as rt

STATE_ATTR = "_w5p01_plugin_state"


def _get_state(config: pytest.Config) -> rt.W5P01PluginState:
    state = getattr(config, STATE_ATTR, None)
    if state is None:
        state = rt.W5P01PluginState()
...
def pytest_collection_modifyitems(
    ...
    except rt.W5P01HarnessError as exc:
...
def w5p01_github_postgres_evidence(pytestconfig: pytest.Config) -> rt.W5P01GitHubPostgresEvidence:
    if pytestconfig.getoption("collectonly"):
        raise rt.W5P01HarnessError("EVIDENCE_FIXTURE_FORBIDDEN_IN_COLLECT_ONLY")
...
@pytest.fixture(scope="session")
def w5p01_postgres_engine(pytestconfig: pytest.Config):
    if pytestconfig.getoption("collectonly"):
        raise rt.W5P01HarnessError("ENGINE_FIXTURE_FORBIDDEN_IN_COLLECT_ONLY")
    state = _get_state(pytestconfig)
    rt.refuse_schema_start_after_manifest_mismatch(state.manifest_pass)
...
@pytest.fixture()
def db(request: pytest.FixtureRequest, w5p01_postgres_engine) -> Generator[Any, None, None]:
    state = _get_state(request.config)
    connection = None
    outer_transaction = None
    session = None
    try:
        connection = w5p01_postgres_engine.connect()
        ...
        if cleanup_errors and not _prior_primary_failure_exists(request):
            raise rt.W5P01HarnessError(rt.CONTROLLED_FIXTURE_CLEANUP_FAILED)
...
def pytest_sessionfinish(session: pytest.Session, exitstatus: int) -> None:
    ...
    try:
        ...
        out = evidence_dir / (
            "w5p01_collect_only_evidence.json"
            if state.collect_only_mode
            else "w5p01_runtime_evidence.json"
        )
