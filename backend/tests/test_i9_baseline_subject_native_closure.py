...
def test_g26_migration_076_i10_care_network_delivery_exists():
    repo = Path(__file__).resolve().parents[2]
    versions = Path(repo) / "backend" / "alembic" / "versions"
    matches = [p for p in versions.glob("*.py") if p.name.startswith("076_i10_")]
    assert len(matches) == 1
    assert matches[0].name == "076_i10_care_network_delivery_foundation.py"


def test_g27_migration_077_i10_medication_adherence_exists():
    repo = Path(__file__).resolve().parents[2]
    versions = Path(repo) / "backend" / "alembic" / "versions"
...
def test_g29_migration_079_cni_owner_provenance_nullable_exists():
    repo = Path(__file__).resolve().parents[2]
    versions = Path(repo) / "backend" / "alembic" / "versions"
    matches = [p for p in versions.glob("*.py") if p.name.startswith("079_i10_cni_")]
    assert len(matches) == 1
    assert matches[0].name == "079_i10_cni_owner_provenance_nullable.py"
